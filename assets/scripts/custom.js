...
class Scene {
    ...
    async init() {
      try {
        await this.loadScript(`https://cdnjs.cloudflare.com/ajax/libs/gsap/2.1.3/TweenMax.min.js`);
        await this.loadScript(`https://cdnjs.cloudflare.com/ajax/libs/gsap/2.1.3/TimelineMax.min.js`);
    ...
    async createTimelineIntro() {
      return new Promise((resolve, reject) => {
        ...
        this.timelineIntro.to(this.indi.underline, 0,
        { xPercent: -100 });
  
  
        this.timelineIntro.to(this.indi.container, 0,
        { opacity: 1 });
  
  
        this.timelineIntro.to(this.indi.underline, 0.3,
        { xPercent: 0, delay: 0.9 });
  
  
        this.timelineIntro.to(this.indi.all, 0.3,
        { xPercent: -100, delay: 9 });
    ...
    async createTweenGridX() {
      return new Promise((resolve, reject) => {
        // Create tween
        this.tweenGridX = TweenMax.fromTo(this.gridX, 1,
        { xPercent: -8 },
    ...
    async createTweenFrame() {
      return new Promise((resolve, reject) => {
        // Create tween
        this.tweenFrame = TweenMax.fromTo(this.frame, 1,
        { rotationZ: 8 },
    ...
    async createTweenOverlay() {
      return new Promise((resolve, reject) => {
        // Create tween
        this.tweenOverlay = TweenMax.fromTo(this.overlay, 1,
        { rotationZ: 1 },
    ...
    async bindMousemove() {
      return new Promise((resolve, reject) => {
        // Cheap test for IE
        if (typeof CSS.supports === 'undefined' || !CSS.supports || !CSS.supports('will-change: transform')) {
          resolve();
        }
